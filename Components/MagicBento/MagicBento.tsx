...
const DEFAULT_GLOW_COLOR = "255, 101, 70";
...
const cardData: Array<BentoCardProps & { type?: string; extra?: any }> = [
  {
    ...
    extra: {
      code: "$ learning on",
      tip: "Tip: Files ending with '/' are directories. Use 'ls -l' for detailed info.",
    },
  },
  {
    color: "#0a0a0a",
    ...
    type: "small",
    extra: {
      list: ["Command suggestions", "File path completion", "History navigation"],
    },
  },
    {
    color: "#0a0a0a",
  ...
  },
  {
    ...
    extra: {
      code: "$ explain ls",
      explanation:
        "ls - List Directory Contents\nWhat it does: Lists all files and folders\nCommon uses: ls -l, ls -a, ls -lh",
    },
  },
];
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {cardData.map((card, index) => {
            // apply size classes based on card.type to preserve cuboid layout
            // use `lg:` so spans apply at the same breakpoint the CSS uses for 4-column layout
            const sizeClass =
              card.type === "large"
            ...
            const minH = card.type === "small" ? "min-h-[220px]" : "min-h-[320px]";

            const baseClassName = `card ${sizeClass} flex flex-col justify-between relative aspect-[4/3] ${minH} w-full max-w-full p-5 rounded-[20px] border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${
                    ...
                    return (
                      <>
                        ...
                        <div className="card__content flex flex-col relative text-white">
                          ...
                          <code className="mt-3 block text-sm">{card.extra?.code}</code>
                          <div className="tip-preview mt-2 text-sm opacity-90">{card.extra?.tip}</div>
                        </div>
                      </>
                    );
                  case "medium":
                    ...
                    return (
                      <>
                        ...
                        <div className="card__content flex flex-col relative text-white">
                          ...
                          {card.extra?.code && <code className="mt-3 block text-sm">{card.extra.code}</code>}
                          {card.extra?.error && (
                            <div className="error-example mt-2 text-sm whitespace-pre-wrap">{card.extra.error}</div>
                          )}
                        </div>
                      </>
                    );
